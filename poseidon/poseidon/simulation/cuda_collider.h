...
template <typename T> class Collider3 {
public:
  /// \param v
  void setVelocity(const hermes::cuda::Vector3<T> &v) { velocity = v; }
  /// \param p
  /// \return true
  /// \return false
  virtual __host__ __device__ bool
  intersect(const hermes::cuda::Point3<T> &p) const = 0;
  /// \param p
  /// \return T
  virtual __host__ __device__ T distance(const hermes::cuda::Point3<T> &p,
                                         hermes::cuda::Point3<T> *s) = 0;

  hermes::cuda::Vector3<T> velocity;
};
...
template <typename T> class Collider3Set : public Collider3<T> {
public:
  __host__ __device__ Collider3Set(Collider3<T> **l, size_t n)
      : list(l), n(n) {}
  __host__ __device__ bool
  intersect(const hermes::cuda::Point3<T> &p) const override {
    for (size_t i = 0; i < n; i++)
      if (list[i]->intersect(p))
        return true;
    return false;
  }
  __host__ __device__ T distance(const hermes::cuda::Point3<T> &p,
                                 hermes::cuda::Point3<T> *s) override {
    T mdist = hermes::cuda::Constants::greatest<T>();
    bool negative = false;
    hermes::cuda::Point3<T> cp;
    for (int i = 0; i < n; i++) {
      T dist = list[i]->distance(p, &cp);
      if (abs(dist) < mdist) {
        mdist = abs(dist);
...
template <typename T> class SphereCollider3 : public Collider3<T> {
public:
  __host__ __device__ SphereCollider3(const hermes::cuda::Point3<T> &center,
                                      T radius)
      : c(center), r(radius) {}
  __host__ __device__ bool
  intersect(const hermes::cuda::Point3<T> &p) const override {
...
template <typename T> class BoxCollider3 : public Collider3<T> {
public:
  __host__ __device__ BoxCollider3(const hermes::cuda::BBox3<T> &box)
      : box(box) {}
  __host__ __device__ bool
  intersect(const hermes::cuda::Point3<T> &p) const override {
