...
void TestFLIP::testApplyForces() {
	...
	}
	{
		...
		for(size_t i = 0; i < particles.size(); i++) {
			CPPUNIT_ASSERT(particles[i]->velocity == ponos::vec3(0, 1, 2));
		}
	}
	...
	flip.applyForces();
	{
		auto particles = flip.particleGrid->getParticles();
		for(size_t i = 0; i < particles.size(); i++) {
			CPPUNIT_ASSERT(particles[i]->velocity == ponos::vec3(2, 2, 5));
		}
	}
}
...
void TestFLIP::testStoreParticleVelocities() {
	...
	}
	{
		auto particles = flip.particleGrid->getParticles();
		for(size_t i = 0; i < particles.size(); i++) {
			FLIPParticle *p = static_cast<FLIPParticle*>(particles[i]);
