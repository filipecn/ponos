...
namespace poseidon {
	...
	VDBLevelSet::VDBLevelSet(const ponos::RawMesh *m, ponos::Transform t) {
		...
		size_t elementCount = m->indices.size() / m->elementSize;
		std::cout << m->verticesIndices.size() << std::endl;
		for(size_t i = 0; i < elementCount; i++){
			ponos::uivec4 face(m->elementSize, &m->verticesIndices[i * m->elementSize]);
		...
		grid = openvdb::tools::meshToLevelSet<openvdb::FloatGrid> (*transform, points, faces);
	}
	...
	float VDBLevelSet::operator()(const float& x, const float &y, const float& z) const {
		float value;
	...
	const openvdb::FloatGrid::Ptr& VDBLevelSet::getVDBGrid() const {
		return grid;
	}
