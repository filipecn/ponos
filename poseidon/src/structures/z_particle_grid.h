			...
			public:
              ...
              return &grid.particles[cur];
				...
				class tree {
					public:
						...
						void searchAndApply(const ponos::Point2& p, float radius, FilterType& f) {
							const ponos::BBox2D bbox(p - ponos::vec2(radius), p + ponos::vec2(radius));
				...
				template<typename T>
					struct WeightedAverageAccumulator {
						...
						void operator()(const T distSqr, ParticleObject* p) {
							...
							float value = 0.f;
							switch(attribute) {
								case ParticleAttribute::VELOCITY_X: value = p->velocity.x; break;
								case ParticleAttribute::VELOCITY_Y: value = p->velocity.y; break;
								case ParticleAttribute::DENSITY: value = p->density; break;
								default: break;
							}
							weightSum += w;
							valueSum += w * value;
				...
				particle_iterator getCell(const ponos::ivec2& ij) {
					particle_iterator it(*this, computeIndex(ponos::Point2(ij[0], ij[1])), maxDepth);
					return it;
				}
				 ...
				 * @r **[in]** radius (ex: value is **1.5 * cellSize**
