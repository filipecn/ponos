...
class ContactDemo : public Demo {
	public:
      ...
      for(int i = 0; i < 5; i++) {
        ...
        ponos::Point2(),
        std::min(0.1f, static_cast<float>(rand() % (width / 2) + 10) / static_cast<float>(width))));
			}
      for (int i = 0; i < 10; i++) {
    ...
    void draw() override {
      ...
      glColor4f(0,0,0,0.1);
      for(hercules::cds::AABBSweep<hercules::_2d::RigidBody>::iterator it(objects); it.next(); ++it)
        aergia::draw_circle(*static_cast<const ponos::Circle*>((*it)->getShape()), &(*it)->transform);
      for(size_t i = 0; i < contacts.size(); i++) {
        ...
        const ponos::Circle& A = *static_cast<const ponos::Circle*>(bA->getShape());
        const ponos::Circle& B = *static_cast<const ponos::Circle*>(bB->getShape());
        ...
        glBegin(GL_LINES);
          aergia::glVertex(contacts[i].points[0].position);
          aergia::glVertex(contacts[i].points[0].position + contacts[i].points[0].penetration * ponos::vec2(contacts[i].normal));
        glEnd();
        //std::cout << "collision " << *static_cast<int*>(bA->userData) << " " << *static_cast<int*>(bB->userData) << std::endl;
      }
