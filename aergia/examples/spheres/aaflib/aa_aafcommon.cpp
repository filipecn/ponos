 ...
 ************************************************************/
AAF::AAF(double v0)
    : cvalue(v0), length(0), size(0),
#ifdef FAST_RAD
      radius(0.0),
#endif
      deviations(NULL), indexes(NULL) {
#ifdef CLEANUP
...
AAF::AAF(double v0, const double *t1, const unsigned *t2, unsigned T)
    : cvalue(v0), length(T), size(T),
#ifdef FAST_RAD
      radius(0.0),
#endif
      deviations(new double[T]), indexes(new unsigned[T]) {
 ...
 ************************************************************/
AAF::AAF(const AAF &P)
    : cvalue(P.cvalue), length(P.length), size(P.size),
#ifdef FAST_RAD
      radius(P.radius),
#endif
      deviations(NULL), indexes(NULL) {
...
AAF::AAF(const AAInterval iv)
    : cvalue((iv.gethi() + iv.getlo()) / 2), length(1), size(1),
#ifdef FAST_RAD
      radius(0.0),
#endif
      deviations(new double[1]), indexes(new unsigned[1]) {
  ...
  for (unsigned i = 0; i < length; i++) {
    if (fabs(deviations[i]) < 1 && fabs(P.deviations[i]) < 1) {
      ...
      if (fabs(deviations[i] - P.deviations[i]) /
                  (fabs(deviations[i]) + fabs(P.deviations[i])) >
              1E-15 ||
          (indexes[i] != P.indexes[i])) {
        result = false;
        break;
 ...
 ************************************************************/
double AAF::getMax() const { return getcenter() + rad(); }
 ...
 ************************************************************/
double AAF::getMin() const { return getcenter() - rad(); }
...
{
  if ((((getcenter() + rad()) > 0) && ((getcenter() - rad()) > 0)) ||
      (((getcenter() + rad()) < 0) && ((getcenter() - rad()) < 0)))
    if (fabs(getcenter() - rad()) > fabs(getcenter() + rad()))
      return fabs(getcenter() + rad());
    else
      return fabs(getcenter() - rad());
...
// the "last" member variable has got to be changed to prevent an uncontrolled
// increase
#endif
  ...
  // now reconstruct all affine variables to contain only relevant deviation
  // terms
  for (iter = allAAF.begin(); iter != allAAF.end(); iter++) {
#ifdef CLEANUP_DEBUG
    ...
    cout << "Found " << number_of_signifcant
         << " significant deviation symbol(s)." << endl;
#endif
