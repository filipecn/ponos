 ...
 *   Output : AAF       : AAF result
 ************************************************************/
AAF operator/(double cst, const AAF P) { return inv(P) * cst; }
...
AAF AAF::operator^(const int exponent) const {
  ...
  if ((b * a < EPSILON) && (exponent < 0)) {
    throw(AAF_Exception(
        AAF_DIVZERO_EXCEPTION,
        std::string("Error <AAF::^>: computing zero inversion").c_str(), a, b));
  }
  ...
  if (AAF::approximationType == CHEBYSHEV) {
    ...
    double x_1 =
        -pow(fabs((double)alpha / exponent), (double)1.0 / (exponent - 1.0));
    ...
    // exp > 0 and exp even and [a,b] > 0 : MINRANGE: alpha = f'(a), y0_b > y0_a
    // : e1 = 1 e2 = 1 e3 = 1
    // exp > 0 and exp odd  and [a,b] > 0 : MINRANGE: alpha = f'(a), y0_b > y0_a
    // : e1 = 1 e2 = 0 e3 = 1
    // exp > 0 and exp even and [a,b] < 0 : MINRANGE: alpha = f'(b), y0_a > y0_b
    // : e1 = 1 e2 = 1 e3 = 0
    // exp > 0 and exp odd  and [a,b] < 0 : MINRANGE: alpha = f'(b), y0_b > y0_a
    // : e1 = 1 e2 = 0 e3 = 0
    // exp < 0 and exp even and [a,b] > 0 : MINRANGE: alpha = f'(b), y0_a > y0_b
    // : e1 = 0 e2 = 1 e3 = 1
    // exp < 0 and exp odd  and [a,b] > 0 : MINRANGE: alpha = f'(b), y0_a > y0_b
    // : e1 = 0 e2 = 0 e3 = 1
    // exp < 0 and exp even and [a,b] < 0 : MINRANGE: alpha = f'(a), y0_b > y0_a
    // : e1 = 0 e2 = 1 e3 = 0
    // exp < 0 and exp odd  and [a,b] < 0 : MINRANGE: alpha = f'(a), y0_a > y0_b
    // : e1 = 0 e2 = 0 e3 = 0
...
AAF aaf_pow(const AAF &base, const AAF &exp) {
  ...
  // Variables
  double fx, fy, fc;
  double dmin, dmax, x1, x2, x3, y1, y2, y3, phi, phi0, phi_max, d1, d2, d3,
      dyc;
  unsigned *merge;
  unsigned *help;
  double *dev_array_b;
  ...
  if (min_b <= 0) {
    throw(AAF_Exception(
        AAF_NEGBASE_EXCEPTION,
        std::string("Error <AAF::aaf_pow>: Exponentiation of negative numbers "
                    "is only possible for integer exponents and currently not "
                    "defined in this scope.").c_str(),
        min_b));
  }
  ...
  // Merge vector of indexes and construct array for the deviations and the
  // epsilon
  // TODO: Pr�fen: mehrere unabh�ngige Abweichungssymbole, die nur in einer AAF
  // vorkommen? - sollte egal sein
  help = new unsigned[length_b + length_e + 1];
  merge =
      std::set_union(ind_b, ind_b + length_b, ind_e, ind_e + length_e, help);
  ...
  for (i = 0; i < 2 * (length_r - 1); i++) {
    ...
    if ((d1 < d2) && (d2 < d3)) {
      ...
      fprintf(stdout, "\t Warning: The distance function is not monotone on "
                      "the straight line from (%f,%f) to (%f,%f) \n",
              x1, y1, x3, y3);
...
AAF sqrt(const AAF &P) {
  ...
  if (a < EPSILON) {
    throw(AAF_Exception(
        AAF_NEGROOT_EXCEPTION,
        std::string("Error <AAF::sqrt>: computing negative root").c_str(), a,
        b));
  }
...
AAF isqrt(const AAF &P) {
  ...
  if ((a < EPSILON) || (b < EPSILON)) {
    throw(AAF_Exception(
        AAF_NEGROOT_EXCEPTION,
        std::string("Error <AAF::isqrt>: computing negative root").c_str(), a,
        b));
  }
...
AAF inv(const AAF &P) {
  ...
  if (a * b < EPSILON) {
    throw(AAF_Exception(
        AAF_DIVZERO_EXCEPTION,
        std::string("Error <AAF::inv>: computing zero inversion").c_str(), a,
        b));
  }
...
AAF log(const AAF &P) {
  ...
  if ((a < EPSILON) || (b < EPSILON)) {
    throw(AAF_Exception(
        AAF_NEGLOG_EXCEPTION,
        std::string("Error <log(AAF)>: computing negative log").c_str(), a, b));
  }
