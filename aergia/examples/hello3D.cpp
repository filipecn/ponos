...
void button(int button, int action) {
  if (action == GLFW_RELEASE)
    trackball.buttonRelease(*cameraview.camera.get(), button,
                            cameraview.getMouseNPos());
  else
    trackball.buttonPress(*cameraview.camera.get(), button,
                          cameraview.getMouseNPos());
}
...
void renderWorldView() {
  grid.draw();
  aergia::CameraModel::drawCamera(
      *static_cast<aergia::Camera *>(cameraview.camera.get()));
  trackball.draw();
}
