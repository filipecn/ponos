...
public:
  ...
  static void drawCamera(const CameraInterface &camera) {
    ...
    float talpha = tanf(
        dynamic_cast<const PerspectiveProjection *>(camera.getCameraProjection())->fov /
        2.f);
    float tbeta = tanf(
        (dynamic_cast<const PerspectiveProjection *>(camera.getCameraProjection())->fov /
    ...
    fbl = camera.getPosition() + dir * camera.getFar() - camera.getFar() * talpha * left -
          camera.getFar() * tbeta * up;
    fbr = camera.getPosition() + dir * camera.getFar() + camera.getFar() * talpha * left -
          camera.getFar() * tbeta * up;
    ftl = camera.getPosition() + dir * camera.getFar() - camera.getFar() * talpha * left +
          camera.getFar() * tbeta * up;
    ftr = camera.getPosition() + dir * camera.getFar() + camera.getFar() * talpha * left +
          camera.getFar() * tbeta * up;
    glColor3f(0, 0, 0);
    glBegin(GL_LINES);
