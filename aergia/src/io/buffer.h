...
namespace aergia {
	...
	struct BufferDescriptor {
		 ...
		 */
		BufferDescriptor( GLuint _elementSize,
	...
	inline 	BufferDescriptor create_vertex_buffer_descriptor(size_t elementSize, size_t elementCount,
	...
	inline 	BufferDescriptor create_index_buffer_descriptor(size_t elementSize, size_t elementCount,
			GLuint elementType = GL_TRIANGLES) {
		return BufferDescriptor(
				elementSize,
				elementCount,
	...
	template<typename T>
		class Buffer {
			public:
				 ...
				 */
				Buffer(const T *d, const BufferDescriptor &bd)
					: Buffer() {
						set(d, bd);
					}
				virtual ~Buffer() {
				...
				void set(const T *d, const BufferDescriptor &bd) {
					...
					glBufferData(bufferDescriptor.type, bufferDescriptor.elementCount * bufferDescriptor.elementSize * sizeof(T), data.get(), bufferDescriptor.use);
				}
				/* bind
				 * Activate buffer
				 */
				...
				void registerAttribute(const std::string &name, GLint location) {
					if(bufferDescriptor.attributes.find(name) == bufferDescriptor.attributes.end())
						return;
					const BufferDescriptor::Attribute &va = bufferDescriptor.attributes[name];
					glVertexAttribPointer(location, bufferDescriptor.elementSize, va.type, GL_FALSE, va.size, (void*)(va.offset));
				}
				// buffer description
