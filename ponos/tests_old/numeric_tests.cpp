...
TEST(Interpolation, linear) {
  ...
  { // 2D
    ...
    for (int i = 0; i < 1000; ++i) {
      ...
      EXPECT_NEAR(bilerp<float>(p.x, p.y, f(0.00, 0.00), f(dx, 0.00), f(dx, dx),
                                f(0.00, dx)),
                  f(p.x * dx, p.y * dx), 1e-6);
    }
    { // 3D
      // TODO
    }
...
TEST(Interpolation, monotonicCubic) {
  { // 1D test
    ...
    for (float s = 0.0; s <= 1.0; s += 0.01) {
      EXPECT_NEAR(
          monotonicCubicInterpolate(f(-1 * dx), f(0), f(1 * dx), f(2 * dx), s),
          f(s * dx), 1e-7);
    }
  }
  { // 2D test
...
TEST(Grid, Accessor) {
  { // 2D
    ...
    EXPECT_NEAR(g.accessor().cellPosition(point2(3.1 * dx, 8.7 * dx)).x, 0.1,
                1e-6);
    EXPECT_NEAR(g.accessor().cellPosition(point2(3.1 * dx, 8.7 * dx)).y, 0.7,
                1e-6);
