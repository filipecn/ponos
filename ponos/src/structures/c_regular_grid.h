...
template <typename T>
T CRegularGrid2D<T>::dSample(float x, float y, T r) const {
...
template <typename T> T CRegularGrid2D<T>::safeData(int i, int j) const {
  return data[std::max(0, std::min(static_cast<int>(this->width) - 1, i))]
             [std::max(0, std::min(static_cast<int>(this->height) - 1, j))];
}
