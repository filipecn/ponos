...
namespace ponos {
	...
	BAH::BAH(Mesh2D *m) {
		...
		for(size_t i = 0; i < mesh->getMesh()->elementCount; i++)
			buildData.emplace_back(BAHElement(i, mesh->getMesh()->elementBBox(i)));
		uint totalNodes = 0;
		orderedElements.reserve(mesh->getMesh()->elementCount);
		root = recursiveBuild(buildData, 0, mesh->getMesh()->elementCount, &totalNodes, orderedElements);
	...
	BAH::BAHNode* BAH::recursiveBuild(std::vector<BAHElement>& buildData, uint32_t start, uint32_t end,
			...
			if(intersect(node->bounds, r, invDir, dirIsNeg)) {
				if(node->nElements > 0) {
					...
					for(uint32_t i = 0; i < node->nElements; i++) {
						ponos::Point2 v0 = mesh->getMesh()->vertexElement(orderedElements[node->elementsOffset + i], 0).xy();
						ponos::Point2 v1 = mesh->getMesh()->vertexElement(orderedElements[node->elementsOffset + i], 1).xy();
