...
namespace ponos {
	 ...
	 * @end **[in]**
	 * @f **[in]**
	 * @gs **[in | optional]** grain size
	 * Breaks the interval **[start,end)** into chunks and call **f** for each chunk in parallel. The size
	 * of each chunk is given by **gs** (or is automatically computed if not given).
	 */
	void parallel_for(size_t start, size_t end, std::function<void(size_t first, size_t last)> f, int gs = -1);

	/* query
	 * @start **[in]** first element index
	 * @end **[in]** last + 1 element index
	 * @array **[in]** array of elements
	 * @gs **[in | optional]** grain size
	 *
	...
	/*		if(start >= end)
			...
			const uint num_threads = std::min(hardware_threads != 0 ? hardware_threads : 2, max_threads);
