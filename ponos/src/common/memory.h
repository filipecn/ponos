...
#define ALLOCA(TYPE, COUNT) (TYPE *)alloca((COUNT) * sizeof(TYPE))
	...
	class MemoryArena {
		public:
			/* Constructor.
			 * @bs **[in]** amount of bytes per region to be allocated (<blockSize>).
			 */
			...
			/* allocation
			 * @sz **[in]** block size to be allocated.
			 * @return pointer to the allocated area. If there is not enough space.. // TODO
			 * Allocates **sz** bytes.
			 */
	...
	/* blocked 2D arrays
	 * Implements a generic 2D array of values. Rather than storing values so that entire
	 * rows are contiguous in memory, the data is ordered in memory using a _blocked_
